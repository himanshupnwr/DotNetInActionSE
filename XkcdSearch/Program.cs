...
namespace XkcdSearch
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var results = Parser.Default.ParseArguments<Options>(args).WithParsedAsync<Options>(
                async options =>
                {
                    ...
                    stopwatch.Stop();
                    Console.WriteLine("Result returned in " +
                      $"{stopwatch.ElapsedMilliseconds} ms");

                    if (comic == null)
                    {
                        Console.WriteLine($"xkcd comic with title " + $"\"{options.Title}\" not found");
                    }
                    else
                    {
                        Console.WriteLine($"xkcd \"{options.Title}\" is number " + $"{comic.Number} published on {comic.Date}");
                    }
                }).Result;
